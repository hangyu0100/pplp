...
#if defined(VERIMAG_POLYHEDRA_MINIMIZE_OPENMP)
  std::mutex unsure_cons_lock ;
#endif
...
Raytracing::Raytracing(Polyhedron& poly, const Point& point, bool getWitness,
      bool cone) : _polyptr(&poly), _start_point(&point), _hasInclusion(true),
...
// the TBB version caused a seg fault, don't know why yet
/*
#ifdef VERIMAG_POLYHEDRA_MINIMIZE_TBB
...
std::vector<int> Raytracing::GetIntersections(const Ray& ray, int currIdx) {
  ...
  double currIdxDis = GetDistanceInverse(currIdx, products(currIdx), thresholdDot) ;
  ...
  for (int i = 0; i < consNum; ++ i) {
    ...
    currDistance = GetDistanceInverse(i, consDirect, threshold) ;
...
*******************************************************************************/
double Raytracing::GetDistanceInverse(const int currIdx, const double consDirect, double threshold) {
...
double Raytracing::GetDistanceInverse(const Vector& cons,
  ...
  double threshold = Tool::GetDotProductThreshold( cons, ray.get_direction() ) ;
...
bool Raytracing::CheckRedundant(const int currIdx, std::vector<int>& headIdx) {
  ...
  log_mtx_raytracing.unlock() ; 
    ...
    Point newPoint = glpkinter.GetIrddWitness(headIdx, *_polyptr, _cone) ;
  ...
  log_mtx_raytracing.unlock() ; 
      ...
      if ( glpkinter.is_result_unsure() ) {

#ifdef DEBUGINFO_RAYTRACING
  log_mtx_raytracing.lock() ; 
  std::cout << "Add constraint " << currIdx << " as unsure" << std::endl ;
  log_mtx_raytracing.unlock() ; 
#endif

#if defined(VERIMAG_POLYHEDRA_MINIMIZE_OPENMP)
        unsure_cons_lock.lock() ;
        _polyptr->unsureCons.push_back(currIdx) ;
        unsure_cons_lock.unlock() ;
#else
        _polyptr->unsureCons.push_back(currIdx) ;
#endif
      }
...
#ifdef DEBUGINFO_RAYTRACING
  log_mtx_raytracing.lock() ; 
  std::cout << "head " << currInter[i] << " exist, return as redundant" << std::endl ;
  log_mtx_raytracing.unlock() ; 
#endif

            //return Farkas(currIdx, currRay) ; 
            // if we are not sure, set it as redundant
...
Vector Raytracing::CalWitnessPoint(const Ray& ray, int currIdx) {
  ...
  for (int i = 0; i < consNum; ++ i) {
    ...
    if ( Double::IsLessEq(consDirect, 0.0, threshold) ) continue ;
    thresholdDot = Tool::GetDotProductThreshold( currCons, ray.get_direction() ) ;
    currDistance = GetDistanceInverse(i, consDirect, thresholdDot) ;
...
bool Raytracing::Farkas(int currIdx, const Ray& ray) {
  ...
  Polyhedron lp(0, lpVariNum, lpConsNum) ;
  double curr ;
  // coefficients
  for (int j = 0; j < variNum; ++ j) {
    for (int i = 0, k = 0; i < consNum; ++ i) {
      ...
      lp.SetEqCoef(j, k, curr) ; 
      ++ k ;
    }
    lp.SetEqCoef(j, lpVariNum-1, 0.0) ; 
    curr = _polyptr->GetCoef(currIdx, j) ;
    lp.SetEqConstant(j, curr) ;
  }
  // constants
  for (int i = 0, k = 0; i < consNum; ++ i) {
    if (i == currIdx) continue ;
    curr = _polyptr->GetConstant(i) ; 
    lp.SetEqCoef(lpConsNum-1, k, curr) ; 
    ++ k ;
  }
  lp.SetEqCoef(lpConsNum-1, lpVariNum-1, 1.0) ; 
  curr = _polyptr->GetConstant(currIdx) ;
  lp.SetEqConstant(lpConsNum-1, curr) ;
